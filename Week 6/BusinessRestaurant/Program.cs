    ...
    static void Main(string[] args)
    {
        ...
        try
        {
            ...
            if (answer1 == 1)
            {
                ...
                MySqlTransaction transaction2 = conn.BeginTransaction();
                try
                {
                    using (MySqlCommand sqlCommand1 = new MySqlCommand($"INSERT INTO Customer VALUES ({sqlName1.Value}, {sqlId1.Value}, {dateTime.Date}, {sqlEmail1.Value}, {sqlPhone1.Value})", conn, transaction2))
                    {
                        sqlCommand1.ExecuteNonQuery();
                    }
                    transaction2.Save("Adding customer");
                    transaction2.Commit();
                }
                catch
                {
                ...
                try
                {
                    using (MySqlCommand sqlCommand2 =
                           new MySqlCommand(
                               $"INSERT INTO RestaurantUser VALUES ({users.Count}, {sqlPass1.Value}, {sqlId1.Value})",
                               conn, transaction1))
                    {
                        sqlCommand2.ExecuteNonQuery();
                    }
                    transaction1.Save("Adding user");
                    transaction1.Commit();
                }
                catch
                {
                    ...
                    switch (answer2)
                    {
                        case 1:  
                            ...
                            try
                            {
                                using (MySqlCommand itemCommand = new MySqlCommand(
                                           $"INSERT INTO Item VALUES ({itemMasters.Count}, {sqlItemName1.Value}, {sqlItemCategory1.Value})",
                                           conn, transaction))
                                {
                            ...
                            try
                            {
                                using (MySqlCommand orderCommand = new MySqlCommand(
                                           $"INSERT INTO RestaurantOrder VALUES ({orders.Count}, {sqlItemQuantity1.Value}, {user1.GetUserId()})",
                                           conn, transaction))
                                {
                            ...
                            try
                            {
                                using (MySqlCommand itemMasterCommand = new MySqlCommand(
                                           $"INSERT INTO ItemMaster VALUES ({itemMasters.Count}, {item.GetItemId()}, {order.GetOrderId()})",
                                           conn, transaction))
                                {
                                    itemMasterCommand.ExecuteNonQuery();
                                }
                                ...
                                if (theVariable.GetMasterId() == item.GetItemId())
                                {
                                    ...
                                    try
                                    {
                                        using (MySqlCommand editItem = new MySqlCommand(
                                                   $"UPDATE Item SET qID = {newItemId} WHERE qID = {item.GetItemId()}",
                                                   conn, transaction))
                                        {
                                    ...
                                    try
                                    {
                                        using (MySqlCommand editItemMaster =
                                               new MySqlCommand(
                                                   $"UPDATE ItemMaster SET childID = {newItemId} WHERE masterID = {itemMaster.GetMasterId()} AND childID = {item.GetItemId()}",
                                                   conn, transaction))
                                        {
                                            editItemMaster.ExecuteNonQuery();
                                        }
                            ...
                            try
                            {
                                ...
                                using (MySqlCommand lineCommand = new MySqlCommand(
                                           $"INSERT INTO OrderLine VALUES ({user1.GetUserId()}, {order.GetOrderId()}, {date.Date})",
                                           conn, transaction))
                                {
                            ...
                            while (orderAnswer!.Equals("yes"))
                            {
                                ...
                                {
                                    using (MySqlCommand itemCommand = new MySqlCommand(
                                               $"INSERT INTO Item VALUES ({item.GetItemId()}, {sqlItemName2.Value}, {sqlItemCategory2.Value})",
                                               conn, transaction))
                                    {
                                        itemCommand.ExecuteNonQuery();
                                    }
                                ...
                                try
                                {
                                    using (MySqlCommand itemMasterCommand =
                                           new MySqlCommand(
                                               $"INSERT INTO ItemMaster VALUES ({itemMaster.GetMasterId()}, {item.GetItemId()}, {order.GetOrderId()})",
                                               conn, transaction))
                                    {
                                        itemMasterCommand.ExecuteNonQuery();
                                    }
                            ...
                            else
                            {
                                ...
                                try
                                {
                                           ...
                                           new MySqlCommand("SELECT * FROM Item", conn, sqlTransaction))
                                    {
                                        ...
                                        Console.Write(theCommand.CommandText);
                                    }
                                    ...
                                    if (!breaker)
                                    {
                                        ...
                                        try
                                        {
                                            using (MySqlCommand command = new MySqlCommand(
                                                       $"UPDATE ItemMaster SET toOrder = null WHERE masterID = {itemMaster.GetMasterId()}",
                                                       conn, theTransaction))
                                            {
                                                command.ExecuteNonQuery();
                                            }
                            ...
                            if (!breaker)
                            {
                                ...
                                try
                                {
                                    using (MySqlCommand sqlCommand = new MySqlCommand(
                                               $"DELETE FROM RestaurantOrder WHERE orderID = {order.GetOrderId()}",
                                               conn, theTransaction))
                                    {
                                        sqlCommand.ExecuteNonQuery();
                                    }
                                ...
                                {
                                    using (MySqlCommand sqlCommand = new MySqlCommand(
                                               $"DELETE FROM ItemMaster WHERE masterID = {itemMaster.GetMasterId()}",
                                               conn, theTransaction))
                                    {
                                        sqlCommand.ExecuteNonQuery();
                                    }
                                ...
                                for (int j = 0; j < pendings.Count; j++)
                                {
                                    if (penders[j].GetItem().GetItemId() == pendings[j])
                                    {
                                        ...
                                        try
                                        {
                                            using (MySqlCommand theCommand =
                                                   new MySqlCommand(
                                                       $"DELETE FROM Item WHERE qID = {penders[j].GetItem().GetItemId()}",
                                                       conn, theTransaction))
                                            {
                                                theCommand.ExecuteNonQuery();
                                            }
                        ...
                        case 4:
                            ...
                            foreach (Order theItem in orders)
                            {
                                if (theItem.GetOrderId() == orderId1)
                                {
                                    ...
                                    {
                                        using (MySqlCommand sqlCommand = new MySqlCommand(
                                                   $"SELECT orderID FROM RestaurantOrder WHERE orderID = {theItem.GetOrderId()}",
                                                   conn, itsTransaction))
                                        {
                                            sqlCommand.ExecuteNonQuery();
                                            Console.Write(sqlCommand.CommandText);
                            ...
                            foreach (ItemMaster myItem in itemMasters)
                            {
                                if (myItem.GetOrder().Equals(order))
                                {
                                    ...
                                    try 
                                    {
                                        using (MySqlCommand printCommand = new MySqlCommand($"SELECT masterID FROM ItemMatser WHERE masterID = {myItem.GetMasterId()}", conn, itsTransaction))
                                        {
                                            printCommand.ExecuteNonQuery();
                                            Console.Write(printCommand.CommandText);
                                        }
                                    ...
                                    try
                                    {
                                        using (MySqlCommand findingCommand =
                                               new MySqlCommand(
                                                   $"SELECT qID FROM Item WHERE qID = {myItem.GetItem().GetItemId()}",
                                                   conn, itsTransaction))
                                        {
                                            findingCommand.ExecuteNonQuery();
                                            Console.Write(findingCommand.CommandText);
                                        }
                                ...
                                if (t.GetOrder().GetOrderId() == num1)
                                {
                                    ...
                                    try
                                    {
                                        using (MySqlCommand printCommand = new MySqlCommand(
                                                   $"SELECT M.masterID, I.Name, I.Category, O.orderID, O.Quantity FROM Item AS I, ItemMaster AS M, RestaurantOrder AS O WHERE I.qID = {t.GetItem().GetItemId()} AND M.masterID = {t.GetMasterId()} AND O.orderID = {t.GetOrder().GetOrderId()}", conn,
                                                   printingTransaction))
                                        {
                                            printCommand.ExecuteNonQuery();
                                            Console.Write(printCommand.CommandText);
                                    ...
                                    {
                                        using (MySqlCommand command = new MySqlCommand(
                                                   $"SELECT L.userID, U.customerID, L.toOrder, O.Quantity, L.historyDate FROM OrderLine AS L, RestaurantUser AS U, RestaurantOrder AS O WHERE L.toOrder = {t.GetOrder().GetOrderId()} AND L.userID = U.pID AND L.toOrder = O.orderID",
                        ...
                        case 6:
                            ...
                            try
                            {
                                ...
                                {
                                    dangerousCommand.ExecuteNonQuery();
                                }
                        ...
                        case 7:  
                            ...
                            try
                            {
                                ...
                                {
                                    clearingCommand.ExecuteNonQuery();
                                }
                                ...
                                if (myVariable.Equals(user1))
                                {
                                    ...
                                    try
                                    {
                                        using (MySqlCommand printerCommand =
                                               new MySqlCommand(
                                                   $"SELECT C.Name, C.ID, C.DOB, C.Email, C.Phone, U.Password FROM Customer C, RestaurantUser U WHERE C.ID = {myVariable.GetCustomer().GetId()} AND U.pID = {myVariable.GetUserId()}",
                                                   conn, printerTransaction))
                                        {
                                            printerCommand.ExecuteNonQuery();
                                            Console.Write(printerCommand.CommandText);
                                        }
                        ...
                        case 9:
                            ...
                            if (dictionary[0])
                            {
                                try
                                {
                                    using (MySqlCommand deleteCommand = new MySqlCommand(
                                               $"DELETE FROM Order WHERE orderID = {order.GetOrderId()}", conn,
                                               pollingTransaction))
                                    {
                            ...
                            try
                            {
                                using (MySqlCommand pollingCommand = new MySqlCommand(
                                           $"DELETE FROM OrderLine WHERE toOrder = {orderLine.GetOrder().GetOrderId()}",
